...
export default function CourseCard({ id, title, description, lawtype1, lawtype2, price, viewUrl}: CourseCardProps) {
  return (
    <div className="bg-white rounded-3xl shadow-lg overflow-hidden w-full sm:w-[350px] flex flex-col">
      <div className="relative h-[140px] sm:h-[180px] w-full bg-gray-50">
        <Image
          src="/gavel-img.png"
          alt={title}
          fill
          className="object-cover w-full sm:w-[90%] px-2 pt-2"
          priority
        />
      </div>
      <div className="p-3 sm:p-4 flex flex-col flex-grow">
        <h3 className="text-lg sm:text-xl font-semibold text-[#0F4C5C]">{title}</h3>
        <p className="text-gray-600 text-xs sm:text-sm mt-2 mb-3 flex-grow">{description}</p> 
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <div className="flex gap-2">
            ...
            <Link href={`/lawcourses/${id}/enroll`}>
